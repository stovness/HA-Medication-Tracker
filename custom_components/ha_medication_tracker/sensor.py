...
# Standard HA units that can be used as sensor units
_STANDARD_UNITS = {"ml", "mg"}


def _doses_per_day(schedule: dict) -> float:
    """Calculate average doses per day from a schedule dict."""
    ...
    return total_doses / 7 if active_days else 0
...
async def async_setup_entry(
    ...
    entities = [MedicationStockSensor(store, config_entry, med_id)]
...
class MedicationStockSensor(SensorEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        _LOGGER.error("ASYNC_ADDED_TO_HASS: %s stock=%s", self.entity_id, self._store.get_stock(self._med_id))
        await super().async_added_to_hass()
        self.async_write_ha_state()
        _LOGGER.error("STATE_WRITTEN: %s", self.entity_id)
    ...
    def _is_low_stock(self) -> bool:
        ...
        doses_per_day = _doses_per_day(data.get("schedule", {}))
        if doses_per_day == 0:
            return False

        return (stock / stock_per_dose) / doses_per_day <= threshold
...
class MedicationDaysRemainingSensor(SensorEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        stock = self._store.get_stock(self._med_id)
        stock_per_dose = data.get("stock_per_dose", 1)
        doses_remaining = int(stock / stock_per_dose) if stock_per_dose else 0
        return {
...
class MedicationNextDoseSensor(SensorEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        return {
            ATTR_SCHEDULE: get_schedule_summary(self._config_entry.data.get("schedule", {})),
